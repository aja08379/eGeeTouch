...
public class ProgressWheel extends View {
    ...
    public ProgressWheel(Context context, AttributeSet attributeSet) {
        ...
        this.timeStartGrowing = FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.shouldAnimate) {
            ...
            if (this.isSpinning) {
                ...
                float f3 = this.mProgress + ((((float) uptimeMillis) * this.spinSpeed) / 1000.0f);
                this.mProgress = f3;
                if (f3 > 360.0f) {
                ...
                if (!this.linearProgress) {
                    float pow = ((float) (1.0d - Math.pow(1.0f - (this.mProgress / 360.0f), 2.0f))) * 360.0f;
                    f2 = ((float) (1.0d - Math.pow(1.0f - (f7 / 360.0f), 4.0f))) * 360.0f;
                    f7 = pow;
                }
                canvas.drawArc(this.circleBounds, f2 - 90.0f, isInEditMode() ? 360.0f : f7, false, this.barPaint);
            }
            if (z) {
                invalidate();
            }
